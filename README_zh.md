...
- **2026-09-09** ⚡ **币安 USDⓈ-M 永续合约交易面（本地）**：`market_type=usdm` 从只读 Shadow 观察扩展为可交易市场——新增 `binance-futures-paper-trade`（测试网 `testnet.binancefuture.com`）与 `binance-futures-live-trade`（实盘，需 mandate）两个 profile；下单/撤单支持全仓/逐仓（`margin_mode`）、`leverage` 与 `reduce_only`，下单前自动同步杠杆与保证金模式（已有持仓的合约只校验不重复设置，规避 Binance -4067）；读取面（余额/持仓/报价/挂单）与 Shadow 严格观察按 profile 分流，Shadow 行为不变；门禁完全复用现有 mandate（CRYPTO 桶、每日限额、kill-switch 审计），永续符号的流动性 floor 查询自动剥离 `:USDT` 结算后缀、funding 镜像上限按 `max_leverage` 缩放；测试网端点守卫与 ccxt 4.5.76 futures-testnet 弃用均按环境显式放行。实盘下单暂未灰度：保留启用入口 `agent/scripts/binance_futures_live_setup.py`（`status`/`check`/`activate`/`restore`，激活自动备份连接器配置，需 `--yes`）。现货行为与 portfolio 面板不受影响；测试网真网络闭环（限价→挂单→撤单、市价开仓、`reduce_only` 平仓）已实测通过。
